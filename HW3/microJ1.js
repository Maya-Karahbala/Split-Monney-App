...
class Method extends Node {
  ...
  toString() {
     ...
     for ( i=0; i<this.params.length; i++){
        if(this.params[i].left.ind<met.nParams)
          { a+=(this.params[i].toString()+",")  
           l--;}

       }
      for (i=0; i<this.vars.length; i++){
        if(!a.includes(this.vars[i])){
           a+=(this.vars[i]+",") 
           l--;
           if(l==0)break;
        }
           }
     return this.name+"("+a.substring(0, a.length-1)+")";
  }
}

class Variable { //move to Expression
   constructor(m, i) { this.met = m; 
    this.ind = i; }
   fValue() { 
...
function identifier()  {
    ...
    met.vars.push(id)
       //ekleme yapıldı
    if (tok == ASSIGN){
      let va= new Variable(met,met.vars.indexOf(id))
      match(ASSIGN)
      let val=expression();
      met.params.push(new Assignment(va, val))
      return new Assignment(va, val)
    }
